...
function get(url: string, params: any = null): Promise<any> {
...
function post(url: string, args: any = null, params: any = null): Promise<any> {
  return new Promise<any>((resolve) => {
    const headers: any = {};
    if (localStorage.getItem("token")) {
      const accessToken: string = localStorage.getItem("token")!;
    ...
    axios
      .post(constructFullUrl(url), args, { params, headers })
      .then((response) => {
        resolve(response?.data ? response.data : response);
      })
      .catch((error) => {
